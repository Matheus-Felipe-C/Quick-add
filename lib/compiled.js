  ...
  var plugin = {
    ...
    */
    ...
    async _addJournalEntry(app) {
      ...
      const [text, timeStampCheckbox, tag] = result;
    ...
    async _checkIfDailyJotExists(app, tag) {
      ...
      let today2 = dt.toLocaleDateString("en", options);
      today2 = today2.split(",");
      today2[0] += suffix;
      today2 = today2.join();
      let dailyJot = await app.findNote({ name: today2, tags: [tag] });
      if (dailyJot == null) {
        console.log("Could not find daily jot with selected tags, creating new daily jot...");
        dailyJot = this._createDailyJot(app, today2, tag);
      }
      return dailyJot;
    },
    /**
