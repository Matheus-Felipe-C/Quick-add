...
::from [${originalNote.name}](https://www.amplenote.com/notes/${originalNote.uuid})`;
      ...
      "Complete original task from duplicate": async function(app, task) {
        try {
          ...
          const originalTask = noteTasks.filter((task2) => {
            let taskContent = task2.content;
            if (taskContent.trim() === taskTitle.trim()) return task2;
            return false;
          });
          await app.updateTask(originalTask.uuid, { completedAt: Math.floor(Date.now() / 1e3) });
